...
public class WordService implements IWordService {
    ...
    //		final boolean extensionChecked = extensions.contains(extension != null ? extension.toLowerCase() :
    // StringUtils.EMPTY);
    //		if (! extensionChecked) {
    ...
    //		final BufferedImage bi = getImageInfo(image, uploadedInputStream, clientImageQuotaUsed,
    // clientImageMaxUsed, imageEntity);
    //
    ...
    //			final String urlImage = imageBaseurl + FileNameUtils.buildRelativePathById(clientId) +  File.separator
    // +  ftpFileName;
    ...
    //				imageEntity.setUrlThumb(imageBaseurl + FileNameUtils.buildRelativePathById(clientId) +  File
    // .separator + generateImageName + thumbnailExtension);
    //				ftpFiles.add(new FTPFile(generateImageName + thumbnailExtension, imageThumbnail));
    //			}
    //
    //			final boolean uploadedFiles = FTPUpload.uploadFile(ftpServer, ftpPort, ftpLogin, ftpPassword,
    // ftpFiles , String.valueOf(clientId));
    //
    //			if (uploadedFiles) {
    //				if (LOG.isDebugEnabled()) {
    //					LOG.debug("files are uploaded on ftp... trying to write in database the object: " +
    // imageEntity.getName() + imageEntity.getDescription());
    //				}
    ...
    //	 * @param imageEntity
    //	 */
    //	private BufferedImage getImageInfo(final File image, final InputStream uploadedInputStream,
    // final Long clientImageQuotaUsed, final Long clientImageMaxUsed, final Image imageEntity) throws IOException {
    //		if (LOG.isDebugEnabled()) {
    ...
    //
    //			if (LOG.isDebugEnabled()) {
    //				LOG.debug(format("clientImageQuotaUsed:\"{0}\", imageSize:\"{1}\", clientImageMaxUsed:\"{2}\"",
    // clientImageQuotaUsed, imageSize, clientImageMaxUsed));
    //			}
    ...
    //	@Override
    //	@ImgdbDefaultTx
    //	public void putImage(final List<Long> managerIds, final Long imageId, final String name,
